...
function initVersionLifecycleNotice() {
    var currentVer = P || "2026.09.14.01";
    var lastNotifiedVer = GM_getValue("Ex_LastNotifiedVersion");

    // 【方案 A：版本升级后的首次开播欢迎与更新内容提醒】
    if (lastNotifiedVer && lastNotifiedVer !== currentVer) {
        GM_setValue("Ex_LastNotifiedVersion", currentVer);
        setTimeout(function() {
            var msg = `
                <div id="ex-notice-changelog" style="cursor: pointer; padding: 2px 0;">
                    <div style="font-weight: 700; font-size: 13px; margin-bottom: 4px; color: #fff;">🎉 DouyuEx-RL 已升级至 v${currentVer}</div>
                    <div style="font-size: 12px; color: #e2e8f0; line-height: 18px;">✨ UI 界面全面完成 MIUIX 美学重构，三级控制台支持即触即开与精准居中锚定！</div>
                    <div style="margin-top: 6px; font-size: 12px; color: #ff7700; font-weight: 600;">👉 点此查看完整更新日志</div>
                </div>
            `;
            T(msg, "info", { timeout: 8000 });
            setTimeout(function() {
                var noticeBox = document.querySelector(".noticejs-info, .noticejs");
                if (noticeBox && !noticeBox.dataset.boundChangelog) {
                    noticeBox.dataset.boundChangelog = "1";
                    noticeBox.onclick = function() {
                        GM_openInTab("https://github.com/xayahcore/DouyuEx-RL/blob/main/CHANGELOG.md", { active: true });
                    };
                }
            }, 300);
        }, 3000);
