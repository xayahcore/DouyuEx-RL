...
function createSignPanel() {
    ...
    cbs.forEach(function(cb) {
        ...
        cb.addEventListener("change", function() {
            ...
            try { localStorage.setItem("ExSave_SignConfig", JSON.stringify(currentCfg)); } catch(e) {}
        });
    });

    safeBind("#sign-panel-start-btn", "click", async function(e) {
        e.stopPropagation();
        ...
        function appendLog(msg, isSuccess) {
            ...
            if (typeof T === "function") T(msg, isSuccess ? "success" : "info");
        }

        try {
            if (typeof executeSignEngine === "function") {
                await executeSignEngine(currentCfg, appendLog);
            } else if (typeof Wn === "function") {
                await Wn(currentCfg);
            }
            if (statusTag) {
...
function createExUpdatePanel() {
    var currentVer = (typeof P !== "undefined" && P) ? P : ((typeof GM_info !== "undefined" && GM_info.script && GM_info.script.version) ? GM_info.script.version : "2026.09.15.02-preview");
        ...
        try {
            ...
            function postStarReport(rid, type) {
                return fetch("https://www.douyu.com/japi/livebiznc/web/anchorstardiscover/user/task/report", {
                    ...
                    body: "ctn=" + encodeURIComponent(realCtn || "1") + "&type=" + type + "&rid=" + encodeURIComponent(rid)
            ...
            async function followAnchorApi(rid, token) {
                return fetch("/wgapi/livenc/liveweb/follow/add", {
                    ...
                    credentials: "include",
                    headers: { "Content-Type": "application/x-www-form-urlencoded" },
                    body: "rid=" + encodeURIComponent(rid) + "&ctn=" + encodeURIComponent(token || "")
