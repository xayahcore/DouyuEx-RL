...
## 📝 最新版本速递 (v2026.09.26.03)

- **🛡️ 工具条不再"消失"**：直播间页面重绘把工具条整块换掉时，插件会自动把原工具条连同九个按钮原样挂回；同时修复了"进房后插件不加载、刷新一次才好"（加载门槛依赖了一个与插件无关的页面元素，现已改为到点照常加载）。
- **🔧 签到与看播积分修复到位**：活动中心签到此前缺少服务端必需的参数与校验令牌，实际没有生效；看直播积分此前把"可以领了"误当成"已经领了"，所以从不自动领取。两项均已按官方前端的真实请求逐字对齐，实测已能正常签到与自动到账。
- **🐞 其他修复**：点「真实人数 / 今日收入」不再连开两个标签页；单个功能初始化出错不再连累其余 30 多个功能（失败会报出是哪个模块）。
