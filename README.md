...
| **首屏核心加载体积** *(Bundle Size)* | 1.9MB+ (强行静态捆绑 6 大重型库) | **< 200KB (ExLoadLib 按需动态装载)** | **首屏包体降低 89%** |
...
### 📦 轻量化按需装载架构
- **首屏按需动态加载 (ExLoadLib)**：将 `flv.js`、`three.js`、`gif.js`、`xlsx`、`SVGA`、`DOMPurify` 剥离出首屏，仅在用户首次触发对应功能时按需拉取，首屏核心代码体积由 1.9MB+ 骤降至不足 200KB。
...
│   ├── common.js          # 全局公共函数与按需加载器 (ExLoadLib, safeBind 等)
│   └── main.js            # 脚本元数据声明与主程序入口
├── tests/                 # 自动化测试套件 (覆盖 57+ 项单元测试与 15 大端到端集成测试场景)
├── build.js               # 原生打包构建器 (集成 V8 AST 语法核验与 Tree-Shaking)
└── CHANGELOG.md           # 完整版本演进历史单一真源 (SSOT)
```
...
## 📝 最新版本速递 (v2026.09.24.01)

- **✨ 用户卡片「弹幕历史」页内查询**：无需跳出直播间，点击任意用户卡片即可在原位磨砂面板回溯历史弹幕与送礼；完美自适应普通/至尊/礼物展馆 3 种等级卡片原生样式，接口容灾与跨日期串台自愈。
- **🎨 三级菜单 UI 全面统一**：分段胶囊 `.miuix-seg` 落地，抽屉接入 iOS 披露折叠箭头，自绘 SVG 下拉指示器，样式单引擎归并并净减 890+ 行冗余样式。
- **🛡️ 检查更新多源容灾**：改接 GreasyFork 官方权威 meta 端点，并发多源取最新值，彻底攻克 CDN 12 小时硬缓存与 403 漏检。
